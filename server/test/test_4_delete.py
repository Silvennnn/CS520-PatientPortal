...
def test_delete_medical_records():
    for medical_record in medical_records:
        cur_token = None
        cur_medical_record_uuid = medical_record["medical_record_uuid"]
        if medical_record["patient_account_name"] == "test_patient_1":
            cur_token = test_patient_1["access_token"]
        else:
            cur_token = test_patient_2["access_token"]
        response = client.delete(
            url=f"/medicalRecord/deleteMedicalRecordByUUID/?token={cur_token}&medical_record_uuid={cur_medical_record_uuid}",
        )
        assert response.status_code == 200
